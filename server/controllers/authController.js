...
const userController = {
    ...
    signIn: async (req, res) => {
        try {
            const curUser = await userService.getUser({ username: req.body.username });
            ...
            if (req.body.username === '' || req.body.password === '')
                ...
                if (!validPassword) {
                    ...
                    httpHandler.Success(res, { token }, 'Đăng nhập thành công');
                }
            }
        } catch (error) {
    ...
    sendVerify: async (req, res) => {
        try {
            ...
            const response = await mailHandler.sendMail(
                ...
                `<h3>Mã xác nhận tải khoản của bạn là: ${verifyToken}</h3>`,
            );
